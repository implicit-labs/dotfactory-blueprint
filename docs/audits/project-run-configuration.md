...
| Schema and explainability | `_validate_projects` and some top-level config accept unknown keys; domain validators vary; this change exposes placement provenance only | P0: reject unknown keys consistently and offer one dry-run resolver showing effective settings, origins, conflicts and required capabilities before any side effects. A misspelled project field must not look configured. |
